...
- **Hybrid scoring:** top-200 by cosine (prefiltered `is_removed=false`, optional format legality) → rescore. Original weights (`0.55·cosine + 0.25·keyword/mechanic jaccard + 0.10·type-line overlap + 0.10·shared resource signals`) shipped in this PR; see the correction below — they didn't hold up against the real recall gate and were superseded. Filters: `?identity=` (commander color-identity subset), `?format=`, exclude seed + same-name variants. Weights are constants tuned against the golden set.
...
- **Correction (2026-08-05, recall-gate fix):** the first real embed run (OPS.md phase 3) measured recall@10 = 0.23 against this design — badly short of the gate. Diagnosed empirically (not guessed): 70% of the golden set's pairs weren't even in the top-200 cosine candidate pool, confirmed via true exact-cosine rank (HNSW bypassed entirely) to be a genuine embedding-quality limit, not an index-tuning one — famous combo pairs like Isochron Scepter/Dramatic Reversal share essentially no oracle-text vocabulary, so cosine similarity structurally cannot find them; pure hybrid-weight retuning capped out around ~30% recall@10. Fixed by adding a known-combo signal from [Commander Spellbook](https://commanderspellbook.com)'s bulk export (`card_combo_pairs`, `scripts/sync_combos.py`, migrations 0007–0008) — **this does not reverse this doc's founding decision** ("not crowd-sourced decklists," line 5 above; `eval/golden_synergies.yaml`'s header calls the same thing "not decklist co-occurrence"): Commander Spellbook data is rules-based combo cataloging (specific card interactions with documented effects), never popularity/co-occurrence data. Current weights: `0.25·cosine + 0.45·combo + 0.20·mechanic jaccard + 0.05·type-line + 0.05·resources`, where the combo component itself blends known-combo strength with Commander Spellbook's own popularity metric as a tie-breaker among a seed's multiple legitimate combo partners (`COMBO_POPULARITY_WEIGHT` in `scoring.py`). Result: recall@10 = 0.62, recall@25 = 0.73, bad-pair leakage unchanged at 0/10. Two external co-occurrence sources (EDHREC, then Archidekt) were evaluated and rejected first — EDHREC's ToS explicitly prohibits automated bulk access; Archidekt's is gray-area (similar language, tolerated in practice, not clean) — Commander Spellbook's bulk JSON export has no such restriction and is used the same way its own frontend uses it.
