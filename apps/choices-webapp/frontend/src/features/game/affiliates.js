// Outbound "Get the winner" deep links (growth plan §6).
//
// Each platform builds a search URL for the winning choice text. Affiliate
// tracking is an optional per-platform wrapper URL (Impact/aggregator style:
// the wrapper redirects and appends the encoded destination). Env vars are
// unset until program approvals land, so links work untagged from day one.
//
// Platform ids must match LINK_PLATFORMS in backend/game.mjs.
//
// iconPath: 24x24 glyphs vendored from simple-icons (CC0-1.0), simpleicons.org
// v16.24.1. Trademarks belong to their owners; buttons are nominative use
// (see the disclosure rendered next to them). Grubhub's simple-icons asset is
// a full wordmark (illegible at 20px) and OpenTable has none, so both use a
// brand-colored letter monogram instead.

const WRAPPERS = {
  ubereats: import.meta.env.VITE_AFF_WRAP_UBEREATS || "",
  doordash: import.meta.env.VITE_AFF_WRAP_DOORDASH || "",
  grubhub: import.meta.env.VITE_AFF_WRAP_GRUBHUB || "",
  opentable: import.meta.env.VITE_AFF_WRAP_OPENTABLE || "",
};

function wrap(id, url) {
  return WRAPPERS[id] ? WRAPPERS[id] + encodeURIComponent(url) : url;
}
...
export const PLATFORMS = [
  {
    ...
    buildUrl: (winner) =>
      wrap("ubereats", `https://www.ubereats.com/search?q=${encodeURIComponent(winner)}`),
  },
  {
    id: "doordash",
    label: "DoorDash",
    ...
    buildUrl: (winner) =>
      wrap("doordash", `https://www.doordash.com/search/store/${encodeURIComponent(winner)}`),
  },
  {
    id: "grubhub",
    label: "Grubhub",
    ...
    monogram: "G",
    buildUrl: (winner) =>
      wrap("grubhub", `https://www.grubhub.com/search?queryText=${encodeURIComponent(winner)}`),
  },
  {
    id: "opentable",
    ...
    monogram: "O",
    buildUrl: (winner) =>
      wrap("opentable", `https://www.opentable.com/s?term=${encodeURIComponent(winner)}`),
  },
];
